...
# Data Parameters
tf.flags.DEFINE_string("input_text_file", "./data/weibo-remark/test/corpus_validation.txt","Test text data source to evaluate.")
...
tf.flags.DEFINE_string("checkpoint_dir", "./runs/1552742944/checkpoints", "Checkpoint directory from training run")
...
begin=0
# while(begin<len(x_raw) and begin+5000<len(x_raw)):
validate_method( x_raw[begin:len(x_raw)],y_test,max_document_length)
    # begin+=5000

# if(begin>=len(x_raw) and begin+5000<len(x_raw)):
#     validate_method(x_raw[begin:len(x_raw)-1],y_test,max_document_length)
